...
def extractMainLobe(window, M):
    ...
    dft_buf = np.concatenate((w[hM2:], np.zeros(N - M), w[:hM2]))

    # the following code is right too
    # dft_buf = np.concatenate((w, np.zeros(N - M)))
    ...
    # the magnitude spectrum is centered around zero
    # locate the main lobe in the center
    mX = np.concatenate((mX[hN:], mX[:hN]))
    ...
    for i in range(left - 1, -1, -1):
        if mX[i] < mX[left]:
            ...
            break
    right = ai
    for i in range(right + 1, len(mX)):
        if mX[i] < mX[right]:
            right = i
        else:
            break
    return mX[left:right + 1]
